...
[Note from 7/16 run: need to filter SNPs for MAF < 0.05 and fix whatever is going on on chromosome 1 with so many SNPs with high positions]
```{r, eval=F}
# all_SNP_results_MVWalduntrimmed <- readRDS("/workdir/ssv42/mvGWAS/output/mvGWAS_asreml_output/mvGWASwPSY2KASPand5PCs_allSNPresults_0716.RDS") 
# lastrowshouldbe <- tail(grep("S18_", all_SNP_results_MVWalduntrimmed$SNP_ID), n=1)
# all_SNP_results_MVWald <- all_SNP_results_MVWalduntrimmed[1:lastrowshouldbe,]
```
...
OLD CODE BELOW -- has not been updated for 5e.Rmd version


Examine LD structure around top SNPs -- PC version
```{r}
lastrundate <- "0609"
all_SNP_results_df <- readRDS(file=paste0(project_path,
              "output/mvGWAS_asreml_output/MVWald_PCs_allSNPeffects_dfCASS_", lastrundate, ".RDS")) %>%
...
for(i in 1:18) {
  ...
  if(nrow(peak1) != 0 & !is.na(peak1$SNP_ID)){
    LDmatrix_peak1 <- genomat_peaks_i %>% dplyr::select(-c(SNP, ALT, REF, compute_group, CHROM, POS)) %>% #drop metadata non-numeric columns
      t() %>% cor() #calculate correlation matrix
    LDwithpeak1 <- LDmatrix_peak1[,peak1$SNP_ID] #pull out correlation with peak1 SNP
    genomat_peaks_i$LDpeak1 <- LDwithpeak1
  ...
  #check if there is another peak on this chromosome and repeat if so
  peak1_region <- c( peak1$POS - 5000000 , peak1$POS + 5000000)  
  genomat_around_peak1 <- genomat_peaks_i %>% 
       filter(POS > peak1_region[1] & POS < peak1_region[2])
  ...
  if(nrow(peak2) != 0){
    LDmatrix_peak2 <- genomat_peaks_i %>% dplyr::select(-c(SNP, ALT, REF, compute_group, CHROM, POS, distance_peak1, peak1_SNP)) %>% 
      t() %>% cor()
    LDwithpeak2 <- LDmatrix_peak2[,peak2$SNP_ID]
    genomat_peaks_i$LDpeak2 <- LDwithpeak2
...
ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_PCs_LDdecay_CASS_", 
                       format(Sys.time(), "%m%d_%H%M"), ".jpg"))
...
saveRDS(all_SNP_results_peaks, file=paste0(project_path, "output/all_SNP_results_PCs_wpeak_annotations_", format(Sys.time(), "%m%d_%H%M"), ".RDS"))


## Plot Manhattan with joint p-values, colored by LD with peak SNP
ggplot(all_SNP_results_peaks, aes(x=POS, y=-log10(P), color=Rsquared_any)) +
  geom_point(aes(pch=peak_SNP, size=peak_SNP)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
  labs(y="-log10(p)", x="Chromosome physical position") +
  ggtitle(paste0("MLMM GWAS for TC and DM with 5 PCs with LD decay")) +
  guides(pch="none", size="none") +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  facet_grid(. ~ CHROM, scales = "free_x", switch="x") +
    theme(strip.background = element_rect(fill = "white"),
...
ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_Manhattan_PCs_wLD_", 
                       format(Sys.time(), "%m%d_%H%M"), ".jpg"))
```



Zoom in on chromosome 1: with PCs version
```{r}
#do LD calculation for all significant SNPs on chromosome 1:
genomat_ROG <- readRDS(file=paste0(project_path, "data/genomat_restofgenome_CASS.RDS")) %>%
  mutate(CHROM= as.integer(gsub("S", "", SNP) %>% gsub("_[0-9].*", "", .)),
...
LDwithpeak1 <- snp_cor_mat[,peak1$SNP_ID]
LDwithpeak2 <- snp_cor_mat[,peak2$SNP_ID]
if(length(num_sig_snps) < 2){print("need to add in more significant SNPs")}
...
# Plot Manhattan colored by LD with peak SNP
chr1_results_peaks <- genomat_peaks_i %>% mutate(CHROM = as.character(CHROM)) %>%
 mutate(Rsquared_any = case_when(Rsquared_peak1 < Rsquared_peak2 ~ Rsquared_peak2, 
                                 TRUE ~ Rsquared_peak1)) %>%
  mutate(isthispeak1 = case_when(SNP == peak1_SNP  ~ "yes", T ~ "no")) %>%
  mutate(isthispeak2 = case_when(SNP == peak2_SNP ~ "yes", T ~ "no")) %>%
  left_join(all_SNP_results_df %>% filter(CHROM=="1"), .) %>% 
...
saveRDS(chr1_results_peaks, file=paste0(project_path, "output/chr1_results_PCs_wpeak_annotations.RDS"))




## Plot chr 1 Manhattan with joint p-values, colored by LD with peak SNP -- QTL region
# For chr. 1 comparison figure 5
ggplot(chr1_results_peaks %>% filter(POS > 20000000), 
       aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  geom_point(aes(pch=isthispeak1, size=isthispeak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name=
                         expression(textstyle("R"^2*" with starred SNP"))) +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("MLMM GWAS for TC and DM with PCs with LD decay")) +
  guides(pch="none", size="none") +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
   annotate("segment", x = c(19400000, 19600000), xend = c(19600000, 19800000), 
            y = c(-0.1, -0.1), yend = c(0.1, 0.1)) +
  geom_vline(aes(xintercept=24155522), color="orange", lty="dashed") #causal SNP


ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_PCs_Chr1Manhattan_wLD_peak1", 
                       format(Sys.time(), "%m%d_%H%M"), ".jpg"))


## Plot chr 1 Manhattan with joint p-values, colored by LD with peak 2 SNP
ggplot(chr1_results_peaks %>% filter(POS > 20000000), 
       aes(x=POS, y=-log10(P), color=Rsquared_peak2)) +
  geom_point(aes(pch=isthispeak2, size=isthispeak2)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name=
                         expression(textstyle("R"^2*" with starred SNP"))) +
  scale_shape_manual(values = c(19,8)) +
...
ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_PCs_Chr1Manhattan_wLD_peak2", 
                       format(Sys.time(), "%m%d_%H%M"), ".jpg"))



## Plot chr 1 Manhattan with joint p-values, colored by LD with peak SNP -- zoomed in further around QTL region
ggplot(chr1_results_peaks, aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  xlim(c(23000000, 27000000)) +
  geom_point(aes(pch=isthispeak1, size=isthispeak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name=
                         expression(textstyle("R"^2*" with starred SNP"))) +
  scale_shape_manual(values = c(19,8)) +
  ...
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133419, xmax = 24176720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + 
  annotate("rect", xmin = 24053419, xmax = 24256720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + 
  #"what if" PSY2 haplotype region including 100kb upstream
  geom_vline(aes(xintercept=24155522), color="orange", lty="dashed") #causal SNP
...
#PSY2 v6 is 24153419 - 24156720
#with cushion of 100kb on either side: "PSY2 what if" region = 24053419 - 24256720 
#with cushion of 20kb on either side: PSY2 haplotype region used in cluster = 24133419 - 24176720
...
range(chr1_results_peaks_AF$AF) #confirmed allele frequency looks okay
...
Note to self on where I left off on May 15.
I checked the DR2, AF, and imputation status of the SNPs.
The top chr1 SNP actually has DR2 = 0.66 despite that I thought it had been filtered for > 0.75. Therefore I don't know if I should trust it, especially since it's a single helicopter and not a nice clear peak.
Now I'm thinking the first peak is PSY2, the second peak might not be real.
Next I should check if this checks out looking at the QTL region univariate GWAS where no PSY2 covariate was used. I should see what the allele frequencies are in the PSY2 region haplotype SNPs and see if it makes sense that S1_2349333 is at a pretty high MAF while the PSY2 region SNPs are more random. If so, that would support that S1_2349333 is capturing the PSY2 variant. If that's the case, I may want to re-run the GWAS with S1_2349333 as a fixed effect covariate. And, I should get the full univariate GWASs running as well.

Other note: My ROG matrix does not include the IDM SNPs. If I am not using an IDM covariate, I should put these SNPs back in the analysis. Also, they may capture important PSY2 variation.





Are these peaks in LD with the PSY2 region SNPs?
```{r}
#PSY2 v6 is 24153419 - 24156720
...
#original genome matrix before MAF filtering
dosagemat <- readRDS(file= paste0(project_path, "data/DosageMatrix_preMAFfiltering", dataset, ".RDS")) 
dosagemat_t <- t(dosagemat) %>% as.data.frame()
dosagemat_t$SNP <- gsub("_[A-Z]$","", rownames(dosagemat_t))
dosagemat_t$POS <- gsub("S[0-9]*_","", dosagemat_t$SNP) %>% as.numeric()
dosagemat_t$CHROM <- gsub("S","", rownames(dosagemat_t)) %>% gsub("_.*_*", "", .)
dosagemat_t$PSY2region <- NA
dosagemat_t[which(dosagemat_t$SNP %in% PSY2regionSNPs), "PSY2region"] <- "yes"
rownames(dosagemat_t) <- gsub("_[A-Z]$","", rownames(dosagemat_t))

dosagemat_QTLregion <- dosagemat_t %>% filter(CHROM=="1" & POS > 20000000 & POS < 30000000)

# Calculate LD with peak SNPs:

#flag peaks and calculate LD 
dosagemat_peaks_LD <- data.frame(SNP = rownames(dosagemat_QTLregion))

topSNPs_chr1 <- topSNPs %>% filter(CHROM == "1") %>% arrange(P)

#calculate LD between each SNP and the peak 1 SNP
peak1 <- topSNPs_chr1[1,]
  LDmatrix_chr1 <- dosagemat_QTLregion %>% dplyr::select(-c(SNP, POS, CHROM, PSY2region)) %>% #drop metadata non-numeric columns
      t() %>% cor() #calculate correlation matrix
    LDwithpeak1 <- LDmatrix_chr1[,peak1$SNP_ID] #pull out correlation with peak1 SNP
    dosagemat_peaks_LD$LDpeak1 <- LDwithpeak1
    dosagemat_peaks_LD$Rsquared_peak1 <- dosagemat_peaks_LD$LDpeak1^2
    
#calculate LD between each SNP and the peak 2 SNP
peak2 <- topSNPs_chr1[2,]
    LDwithpeak2 <- LDmatrix_chr1[,peak2$SNP_ID]
    dosagemat_peaks_LD$LDpeak2 <- LDwithpeak2
    dosagemat_peaks_LD$Rsquared_peak2 <- dosagemat_peaks_LD$LDpeak2^2

dosagemat_QTL_LD <- right_join(dosagemat_peaks_LD, dosagemat_QTLregion, by="SNP") %>%
  ...
  dplyr::select(SNP, POS, CHROM, PSY2region, peak1SNP, peak2SNP, distance_peak1, distance_peak2, Rsquared_peak1, Rsquared_peak2) %>%
  full_join(chr1_results_peaks %>% mutate(SNP = as.character(SNP)))

dosagemat_QTL_LD[is.na(dosagemat_QTL_LD$P), "P"] <- 1
dosagemat_QTL_LD[is.na(dosagemat_QTL_LD$PSY2region), "PSY2region"] <- "no"
dosagemat_QTL_LD[is.na(dosagemat_QTL_LD$isthispeak1), "isthispeak1"] <- "no"


## Plot chr 1 Manhattan with joint p-values, colored by LD with peak SNP, including SNPs with MAF < 0.05
ggplot(dosagemat_QTL_LD, aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  geom_point(aes(pch=isthispeak1, size=isthispeak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("MLMM GWAS for TC and DM with LD decay")) +
  ...
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  geom_vline(aes(xintercept=24149911), color="orange", lty="dashed") #SNP included in PSY2 hap

## Zooming in on QTL region
ggplot(dosagemat_QTL_LD, aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  xlim(23000000, 27000000) +
  geom_point(aes(pch=isthispeak1, size=isthispeak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("Chr 1 QTL region zoomed in")) +
  guides(pch="none", size="none") +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133741, xmax = 24170185, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + #PSY2 haplotype region
...
ggplot(dosagemat_QTL_LD, aes(x=POS, y=-log10(P), color=Rsquared_peak2)) +
  xlim(23000000, 27000000) +
  geom_point(aes(pch=isthispeak2, size=isthispeak2)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
...
## Zooming in on chr 1 peak 1 QTL region
ggplot(dosagemat_QTL_LD, aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  xlim(24000000, 26800000) +
  geom_point(size=1, aes(pch=isthispeak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ...
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133741, xmax = 24176720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + #PSY2 haplotype region
  annotate("rect", xmin = 24153419, xmax = 24156720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") #PSY2 coding region
        
ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_TCDM_bivariateWaldP_Chr1_QTL_zoomed_peak1", dataset, "_", format(Sys.time(), "%m%d_%H%M"), ".jpg"), width=12, height=5)

## Zooming in on chr 1 PSY2 region
ggplot(dosagemat_QTL_LD, aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  xlim(24000000, 24350000) +
  geom_point(size=1) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
...
# Check if any PVA markers are present in pre-imputed dataset -- no
PVAmarkers <- c("S1_24155522", "S1_24159583", "S1_24160008", "S1_24611696", "S1_24636113", "S1_30543380", "S1_30543962")
PVAmarkers %in% dosagemat_t$SNP


# Check introgression status:
dosagemat_IDMs <- dosagemat_t[rownames(dosagemat_t) %in% IDMlist$SNP,] %>% left_join(IDMlist, by="SNP") %>% filter(GlazFreq == 1 & EscFreq == 0)
#try recalculating allele frequency:
justgenos <- dosagemat_IDMs[,c(1:380)]
AFfunction <- function(x){sum(x)/(ncol(justgenos)*2)}
AFs <- apply(justgenos, MARGIN = 1, FUN = AFfunction)
dosagemat_IDMs$AF <- AFs

```

Which are the non-imputed SNPs?
It seems like the cluster of salmon-colored SNPs just downstream of PSY2 haplotype region might actually be better capturing the ungenotyped causal variant than my region is. So I may need to reconsider how I account for the PSY2 causal variant in my analysis. Perhaps I should do a univariate GWAS for TC in this region and see which SNPs are most associated.



Zoom in on chromosome 10:

```{r}
#do LD calculation for all significant SNPs on chromosome 10:
genomat_peaks_10 <- genomat_ROG %>% filter(CHROM == "10")
rownames(genomat_peaks_10) <- genomat_peaks_10$SNP
  
# pull out the names of any SNPs above the Gao cutoff
topSNPs_chr10 <- topSNPs %>% filter(CHROM == "10") %>% arrange(P)
num_sig_snps <- seq(1, sum(topSNPs_chr10$P < 10^-4.9))
for(i in 1:length(num_sig_snps)){
  assign ( paste0("peak", i), topSNPs_chr10[num_sig_snps[i],] )
}

#calculate LD between each SNP and the peak SNPs
snp_cor_mat <- genomat_peaks_10 %>% dplyr::select(-c(SNP, ALT, REF, compute_group, CHROM, POS)) %>% 
    t() %>% cor()

LDwithpeak1 <- snp_cor_mat[,peak1$SNP_ID]
LDwithpeak2 <- snp_cor_mat[,peak2$SNP_ID]
if(length(num_sig_snps) > 1){print("need to add in more significant SNPs")}
    
genomat_peaks_10 <- genomat_peaks_10 %>% mutate(distance_peak1 = POS - peak1$POS, 
                                              peak1_SNP = peak1$SNP_ID,
                                              LDpeak1 = LDwithpeak1,
                                              Rsquared_peak1 = LDwithpeak1^2, 
                                              distance_peak2 = POS - peak2$POS, 
                                              peak2_SNP = peak2$SNP_ID,
                                              LDpeak2 = LDwithpeak2,
                                              Rsquared_peak2 = LDwithpeak2^2)

#sum(names(LDwithpeak1) != rownames(genomat_peaks_10)) #checked that snp order is consistent so okay to smoosh in vector to dataframe


#Prepare to plot
# Plot Manhattan colored by LD with peak SNP
chr10_results_peaks <- genomat_peaks_10 %>% mutate(CHROM = as.character(CHROM)) %>%
  mutate(isthispeak1 = case_when(SNP == peak1_SNP  ~ "yes", T ~ "no")) %>%
  mutate(isthispeak2 = case_when(SNP == peak2_SNP  ~ "yes", T ~ "no")) %>%
  left_join(all_SNP_results_df %>% filter(CHROM=="10"), .) %>% 
  arrange(as.numeric(CHROM), as.numeric(POS)) %>%
  filter(!duplicated(SNP_ID)) %>%
  mutate(SNP = factor(SNP, levels = SNP))

saveRDS(chr10_results_peaks, file=paste0(project_path, "output/chr10_results_wpeak_annotations.RDS"))


## Plot chr 10 Manhattan with joint p-values, colored by LD with peak SNP
ggplot(chr10_results_peaks, aes(x=POS, y=-log10(P), color=Rsquared_peak1)) +
  geom_point(aes(pch=isthispeak1, size=isthispeak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
...
ggplot(chr10_results_peaks, aes(x=POS, y=-log10(P), color=Rsquared_peak2)) +
  geom_point(aes(pch=isthispeak2, size=isthispeak2)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with starred SNP") +
  scale_shape_manual(values = c(19,8)) +
  scale_size_manual(values=c(0.8,1.2)) +
...
#ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_Chr1Manhattan_wLD_", 
#                       format(Sys.time(), "%m%d_%H%M"), ".jpg"))
...
## Examine allele frequency and imputed SNPs:
chr10_SNP_stats <- read.table(file=paste0(project_path, "data/chr10_YellowGenotypes_REF19imputed_stats.txt"))
colnames(chr10_SNP_stats) <- c("SNP", "POS", "REF", "ALT","DR2","AF","IMP")

chr10_results_peaks_AF <-  left_join(chr10_results_peaks, chr10_SNP_stats, by=c("SNP","POS")) %>%
  dplyr::select_at(vars(-c(contains("IITA"), contains("TMEB"), contains("TMS")))) %>%
  arrange(P)

head(chr10_results_peaks_AF)

```


Are both peaks retained when the other is included as a covariate in the model?
Note: needs to be updated for PSY2cluster only
```{r, eval}
#read in top SNP info
all_SNP_results_MVWald <- readRDS("/workdir/ssv42/mvGWAS/output/mvGWAS_asreml_output/MVWald_allSNPeffects_dfCASS_0608.RDS")
topSNPs <- all_SNP_results_MVWald %>% filter(P < 0.001)
sigSNPs <- all_SNP_results_MVWald %>% filter(P < Gao.cutoff)

  
#get names of significant SNPs
chr1peak1 <- (sigSNPs %>% filter(CHROM==1) %>% arrange(P) %>% dplyr::select(SNP_ID))[1,1]
chr1peak2 <- (sigSNPs %>% filter(CHROM==1) %>% arrange(P) %>% dplyr::select(SNP_ID))[4,1]
chr10peak1 <- (sigSNPs %>% filter(CHROM==10) %>% arrange(P) %>% dplyr::select(SNP_ID))[1,1]
chr10peak2 <- (sigSNPs %>% filter(CHROM==10) %>% arrange(P) %>% dplyr::select(SNP_ID))[2,1]
topSNPsgroup <- c(chr1peak1, chr1peak2, chr10peak1, chr10peak2)
...
BLUPs_hap_data <- left_join(BLUPs_data, PSY2haps, by="germplasmName") %>%
...
# Get starting values to help model
T1 <- asreml(data = BLUPs_hap_data, 
             fixed = TCICHK ~ 1 + PSY2_clusterHapA + and(PSY2_clusterHapB), 
             random = ~ vm(germplasmName, Ginv_ROG),
             residual = ~units)
...
T2 <- asreml(data = BLUPs_hap_data, 
               fixed = DM ~ 1 + PSY2_clusterHapA + and(PSY2_clusterHapB), 
               random = ~ vm(germplasmName, Ginv_ROG),
               residual = ~units)
Vg_DM <- summary(T2)$varcomp$component[1]
...
#select SNPs in the top SNPs group
section_SNPs <- genomat_ROG %>% filter(genomat_ROG$SNP %in% topSNPsgroup)

model_storage <- list(); WaldTest_storage <- list(); SNPeff_storage <- list()
for(i in seq_along(section_SNPs$SNP)){ #this code is written to go through two peaks on chr 1 and two peaks on chr 10
    
  #set main effect SNP
    SNPi <- section_SNPs$SNP[i]; print(SNPi)
    SNPidata <- as.data.frame(t(section_SNPs[i, -c(1:3)]))
    colnames(SNPidata) <- paste0(SNPi)
    SNPidata$germplasmName <- gsub("\\.","-", rownames(SNPidata))
    
  #set covariate SNP
    if(i == 1 | i == 3){j <- i+1} else if(i == 2 | i == 4){j <- i-1}
    CovSNPj <- section_SNPs$SNP[j]
    CovSNPdata <- as.data.frame(t(section_SNPs[j, -c(1:3)]))
    colnames(CovSNPdata) <- paste0(CovSNPj)
    CovSNPdata$germplasmName <- gsub("\\.","-", rownames(CovSNPdata))


    modeldata <- left_join(BLUPs_hap_data, SNPidata, by="germplasmName") %>% 
      left_join(., CovSNPdata, by="germplasmName") %>% 
      mutate(germplasmName = as.factor(germplasmName))
    

    #model calculating SNP effect on each trait
    SNPimodel <- asreml(data = modeldata,
        fixed = formula(paste0("cbind(TCICHK, DM) ~ trait + trait:", SNPi, " + ", CovSNPj, "+ PSY2_clusterHapA + and(PSY2_clusterHapB)")),
         random = ~ corgh(trait, init=initG):vm(germplasmName, Ginv_ROG),
        residual = ~units:corgh(trait), maxit=20)
    ...
    WaldTest_storage[[i]] <- WaldTest$result$chi2
    SNPeff_storage[[i]] <- predictSNPeffects
    
    print("done")
    
}

tosave <-list(model_storage, WaldTest_storage, SNPeff_storage)
saveRDS(tosave, file="./output/topSNPmodelcomparison_0612.RDS")
model_comp <- readRDS(file="./output/topSNPmodelcomparison_0612.RDS")


## Compare chromosome 1 models 
summary(model_comp[[1]][[1]], coef=T)$coef.fixed #main SNP: S1_24309741 (was S1_23493338); covariate = S1_26700297
summary(model_comp[[1]][[1]], coef=T)$aic # lower AIC (-76)
model_comp[[2]][[1]]["P"] #< Gao.cutoff #Wald test: no longer significant with covariate
print(model_comp[[3]][[1]]) #SNP effect direction: +TCICHK -DM

summary(model_comp[[1]][[2]], coef=T)$coef.fixed #main SNP: S1_26700297; covariate = S1_24309741;
summary(model_comp[[1]][[2]], coef=T)$aic # AIC (-75)
model_comp[[2]][[2]]["P"] < Gao.cutoff #still significant
print(model_comp[[3]][[2]]) # #SNP effect direction: +(small) TCICHK -DM

#The chr 1 top SNP does not maintain significance when the other is added into the model as a covariate, but the second SNP maintains significance when the first top SNP is added as a covariate.


## Compare chromosome 10 models 
summary(model_comp[[1]][[3]], coef=T)$coef.fixed #main SNP: S10_2262535; covariate = S10_2846060;
summary(model_comp[[1]][[3]], coef=T)$aic #AIC (-81)
model_comp[[2]][[3]]["P"] < Gao.cutoff #Wald test: significant
print(model_comp[[3]][[3]]) # SNP effects: +TCICHK -(small)DM


## Compare chromosome 10 models 
summary(model_comp[[1]][[4]], coef=T)$coef.fixed #main SNP: S10_2846060; covariate = S10_2262535
summary(model_comp[[1]][[4]], coef=T)$aic #lower AIC (-84)
model_comp[[2]][[4]]["P"] < Gao.cutoff #Wald test: NOT significant
print(model_comp[[3]][[4]]) # SNP effects: +TCICHK -(small)DM


#The top SNP maintains significance when the 2nd highest is added into the model as a covariate.
#The second SNP is just below the significance threshold when the first is added as a covariate.
...
Univariate test for TCICHK associations on chromosome 1 to try to pinpoint PSY2 variant:
```{r}
#select SNPs in this compute group
section_SNPs <- genomat_ROG %>% filter(compute_group > 2 & compute_group <= 5)

Waldtest_storage <- list()
SNPeffs_storage <- list()

for(i in seq_along(section_SNPs$SNP)){ print(i);
  SNPi <- section_SNPs$SNP[i]
  SNPidata <- as.data.frame(t(section_SNPs[i, -c(1:3)]))
  colnames(SNPidata) <- paste0(SNPi)
  SNPidata$germplasmName <- gsub("\\.","-", rownames(SNPidata))
  ...
  covSNP <- "S1_23493338"
  covSNPdata <- as.data.frame(t(section_SNPs[section_SNPs$SNP=="S1_23493338", -c(1:3)]))
  colnames(covSNPdata) <- paste0(covSNP)
  covSNPdata$germplasmName <- gsub("\\.","-", rownames(covSNPdata))

  if(SNPi == covSNP){ #placeholder for when we get to the covariate SNP
    Waldtest_storage[[i]] <- NA; SNPeffs_storage[[i]] <- NA} 
  else{
  ...
  modeldata <- left_join(BLUPs_hap_data, SNPidata, by="germplasmName") %>% 
    ...
    #GWAS model for TCICHK only
    SNPimodel <- asreml(data = modeldata,
        formula(paste0("TCICHK ~ 1 + ", SNPi, " + ", covSNP)),
         random = ~vm(germplasmName, Ginv_ROG),
        residual = ~units, trace=F)
    
    SNPi_Waldtest <- as.data.frame(wald.asreml(SNPimodel))
    Waldtest_storage[[i]] <- SNPi_Waldtest
    SNPeffs_storage[[i]] <- summary(SNPimodel, coef=T)$coef.fixed
  }
}

names(Waldtest_storage) <- section_SNPs$SNP
names(SNPeffs_storage) <- section_SNPs$SNP

saveRDS(Waldtest_storage, file=paste0(project_path,"output/chr1_TCICHKonly_covarS1_23493338_Waldtests_", format(Sys.time(), "%m%d"), ".RDS"))
    
saveRDS(SNPeffs_storage, file=paste0(project_path,"output/chr1_TCICHKonly_covarS1_23493338_SNPeffests_", format(Sys.time(), "%m%d"), ".RDS"))

#combine results from all SNPs into one df
allSNPresults <- data.frame()
for (i in 1:length(Waldtest_storage)){
  SNPiresults <- Waldtest_storage[[i]][2,]
  SNPiresults$SNP <- rownames(SNPiresults)
  SNPieffest <- SNPeffs_storage[[i]][2,]
  if(length(SNPieffest) != 0){
  SNPibothresults <- cbind(SNPiresults, as.data.frame(t(SNPieffest)))
  allSNPresults <- rbind(allSNPresults, SNPibothresults)
  #allSNPresults <- rbind(allSNPresults, SNPiresults)

  }
}
allSNPresults$POS <- as.numeric(gsub("S1_", "", allSNPresults$SNP))
allSNPresults$P <- allSNPresults$`Pr(Chisq)`

allSNPresults %>% arrange(P) %>% head(n=20)


# saveRDS(allSNPresults, file=paste0(project_path, 
#         "output/chr1_TCICHKonly_nocovars_allresults_", 
#         format(Sys.time(), "%m%d"), ".RDS"))


saveRDS(allSNPresults, file=paste0(project_path, 
        "output/chr1_TCICHKonly_covarS1_23493338_allresults_", 
        format(Sys.time(), "%m%d"), ".RDS"))
...
#calculate LD between each SNP and the peak 1 SNP
peak1 <- "S1_23493338"
LDwithpeak1 <- LDmatrix_chr1[,peak1] %>% as.data.frame() #pull out correlation with peak1 SNP from previously calculated LD matrix
colnames(LDwithpeak1) <- "LDpeak1"
LDwithpeak1$SNP <- rownames(LDwithpeak1)

allSNPresults_LD <- left_join(allSNPresults, LDwithpeak1, by="SNP")
allSNPresults_LD$Rsquared_peak1 <- allSNPresults_LD$LDpeak1^2

# saveRDS(allSNPresults_LD, file=paste0(project_path,
#         "output/chr1_TCICHKonly_nocovars_allresults_wLD",
#         format(Sys.time(), "%m%d"), ".RDS"))

saveRDS(allSNPresults_LD, file=paste0(project_path,
        "output/chr1_TCICHKonly_covarS1_23493338_allresults_wLD_",
        format(Sys.time(), "%m%d"), ".RDS"))

```

Univariate test for TCICHK Chr. 1 Manhattan plots
```{r}
allSNPresults_covar <- readRDS(file=paste0(project_path, 
        "output/chr1_TCICHKonly_covarS1_23493338_allresults_wLD_0514.RDS"))

allSNPresults_nocovar <- readRDS(file=paste0(project_path, 
        "output/chr1_TCICHKonly_nocovars_allresults_wLD0514.RDS"))


ggplot(allSNPresults_covar, aes(x=POS, y=-log10(P))) +
  geom_point(size=0.5, aes(color=Rsquared_peak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with SNP covariate") +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("Univariate GWAS for TCICHK with S1_23493338 covariate")) +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133741, xmax = 24170185, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + #PSY2 haplotype region
  annotate("rect", xmin = 24153419, xmax = 24156720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") #PSY2 coding region
...
ggplot(allSNPresults_nocovar, aes(x=POS, y=-log10(P))) +
  geom_point(size=0.5, aes(color=Rsquared_peak1)) +
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with top SNP") +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("Univariate GWAS for TCICHK without covariate")) +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133741, xmax = 24170185, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + #PSY2 haplotype region
  annotate("rect", xmin = 24153419, xmax = 24156720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") #PSY2 coding region
...
allSNPresults_nocovar_AF <-  left_join(allSNPresults_nocovar, chr1_SNP_stats, by=c("SNP","POS"))

head(allSNPresults_nocovar_AF %>% arrange(P))
```



Univariate test for DM associations on chromosome 1 to clarify two peaks:
```{r}
#select SNPs in this compute group
section_SNPs <- genomat_ROG %>% filter(compute_group > 2 & compute_group <= 5)
...
for(i in seq_along(section_SNPs$SNP)){ print(i);
  ...
  covSNP <- "S1_26700297"
  covSNPdata <- as.data.frame(t(section_SNPs[section_SNPs$SNP=="S1_26700297", -c(1:3)]))
  colnames(covSNPdata) <- paste0(covSNP)
  covSNPdata$germplasmName <- gsub("\\.","-", rownames(covSNPdata))
  ...
  modeldata <- left_join(BLUPs_hap_data, SNPidata, by="germplasmName") %>% 
    ...
    #GWAS model for DM only
    SNPimodel <- asreml(data = modeldata,
        formula(paste0("DM ~ 1 + ", SNPi, " + ", covSNP)),
         random = ~vm(germplasmName, Ginv_ROG),
        residual = ~units, trace=F)
...
saveRDS(Waldtest_storage, file=paste0(project_path,"output/chr1_DMonly_covarS1_26700297_Waldtests_", format(Sys.time(), "%m%d"), ".RDS"))
    
saveRDS(SNPeffs_storage, file=paste0(project_path,"output/chr1_DMonly_covarS1_26700297_SNPeffests_", format(Sys.time(), "%m%d"), ".RDS"))

#combine results from all SNPs into one df
allSNPresults <- data.frame()
for (i in 1:length(Waldtest_storage)){
  if(length(Waldtest_storage[[i]]) > 1){
  SNPiresults <- Waldtest_storage[[i]][2,]
  SNPiresults$SNP <- rownames(SNPiresults)
  SNPieffest <- SNPeffs_storage[[i]][2,]
  if(length(SNPieffest) != 0){
...
# saveRDS(allSNPresults, file=paste0(project_path,
#         "output/chr1_DMonly_nocovars_allresults_",
#         format(Sys.time(), "%m%d"), ".RDS"))


saveRDS(allSNPresults, file=paste0(project_path,
        "output/chr1_TCICHKonly_covarS1_26700297_allresults_",
        format(Sys.time(), "%m%d"), ".RDS"))
...
# saveRDS(allSNPresults_LD, file=paste0(project_path,
#         "output/chr1_DMonly_nocovars_allresults_wLD",
#         format(Sys.time(), "%m%d"), ".RDS"))

saveRDS(allSNPresults_LD, file=paste0(project_path,
        "output/chr1_DMonly_covarS1_26700297_allresults_wLD_",
        format(Sys.time(), "%m%d"), ".RDS"))

```

Univariate DM Chr. 1 Manhattan plots
```{r}
allSNPresults_covar <- readRDS(file=paste0(project_path, 
        "output/chr1_DMonly_covarS1_26700297_allresults_wLD_0514.RDS"))

allSNPresults_nocovar <- readRDS(file=paste0(project_path, 
        "output/chr1_DMonly_nocovars_allresults_wLD0514.RDS"))
...
ggplot(allSNPresults_covar, aes(x=POS, y=-log10(P))) +
  ...
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with SNP covariate") +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("Univariate GWAS for DM with S1_26700297 covariate")) +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133741, xmax = 24170185, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + #PSY2 haplotype region
  annotate("rect", xmin = 24153419, xmax = 24156720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") #PSY2 coding region
...
ggplot(allSNPresults_nocovar, aes(x=POS, y=-log10(P))) +
  ...
  scale_color_gradient2(low = "blue", mid="orange", high = "red", midpoint = 0.5, name="R squared with top SNP") +
  labs(y="-log10(p)", x="Chr. 1 physical position (bp)") +
  ggtitle(paste0("Univariate GWAS for DM without covariate")) +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  theme(strip.background = element_rect(fill = "white")) +
  annotate("rect", xmin = 24133741, xmax = 24170185, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") + #PSY2 haplotype region
  annotate("rect", xmin = 24153419, xmax = 24156720, ymin = 0, ymax = 6, alpha = 0.2, fill="orange") #PSY2 coding region
...
Scatterplot of haplotype random effects:
```{r}
generaltest <- asreml(data = modeldata,
        formula(paste0("cbind(TCICHK, DM) ~ trait")),
        #'trait' term gives intercept for each trait
         random = ~ at(trait):vm(germplasmName, Amatrix_notsing) +
           + at(trait):vm(IDMs_HAPA, Amat_haps_IDMs_notsing) +
          and(vm(IDMs_HAPB, Amat_haps_IDMs_notsing)) + 
          at(trait):vm(PSY2_HAPA, Amat_haps_PSY2_notsing) +
          and(vm(PSY2_HAPB, Amat_haps_PSY2_notsing)))
  
modsum <- summary(generaltest, coef=T)



PSY2hap_effects <- modsum$coef.random %>% as.data.frame() %>%
  filter(grepl ("PSY2", rownames(.)) ) %>%
  mutate("trait" = case_when(grepl("TCICHK", rownames(.)) == T ~ "TCICHK", 
                             grepl("DM", rownames(.)) == T ~ "DM")) %>%
  mutate("haplotypeID" = gsub( ".*\\)_", "", rownames(.))) %>%
  pivot_wider(names_from=trait, values_from=c(solution, std.error, z.ratio))

IDMhap_effects <- modsum$coef.random %>% as.data.frame() %>%
  filter(grepl ("IDM", rownames(.)) ) %>%
  mutate("trait" = case_when(grepl("TCICHK", rownames(.)) == T ~ "TCICHK", 
                             grepl("DM", rownames(.)) == T ~ "DM")) %>%
  mutate("haplotypeID" = gsub( ".*\\)_", "", rownames(.))) %>%
  pivot_wider(names_from=trait, values_from=c(solution, std.error, z.ratio))

GID_effects <- modsum$coef.random %>% as.data.frame() %>%
  filter(grepl ("germplasmName", rownames(.)) ) %>%
  mutate("trait" = case_when(grepl("TCICHK", rownames(.)) == T ~ "TCICHK", 
                             grepl("DM", rownames(.)) == T ~ "DM")) %>%
  mutate("germplasmName" = gsub( ".*\\)_", "", rownames(.))) %>%
  pivot_wider(names_from=trait, values_from=c(solution, std.error, z.ratio))



ggplot(PSY2hap_effects, aes(solution_TCICHK, solution_DM)) +
  geom_point() +
  xlab("PSY2 haplotype effect on TCICHK") +
  ylab("PSY2 haplotype effect on DM") +
  ggtitle("Effect estimates of PSY2 haplotypes")  +
  geom_smooth(method="lm")

ggplot(IDMhap_effects, aes(solution_TCICHK, solution_DM)) +
  geom_point() +
  xlab("IDM haplotype effect on TCICHK") +
  ylab("IDM haplotype effect on DM") +
  ggtitle("Effect estimates of glaziovii introgression haplotypes") +
  geom_smooth(method="lm")

ggplot(GID_effects, aes(solution_TCICHK, solution_DM)) +
  geom_point() +
  xlab("GID effect on TCICHK") +
  ylab("GID effect on DM") +
  ggtitle("Effect estimates of germplasmName") +
  geom_smooth(method="lm")


#error bars on haplotype effects are so big it makes the points cluster into one

  # geom_errorbar(aes(ymin=solution_DM-(std.error_DM/370), ymax=solution_DM+(std.error_DM/370)),
  #                   width=0.0000001) +
  # geom_errorbar(aes(xmin=solution_TCICHK-(std.error_TCICHK/370), xmax=solution_TCICHK+(std.error_TCICHK/370)),
  #               width=0.0000001)
```
...
# Run associations with TCICHK and DM with PSY2 and Glaz Introgression as random effects, multivariate test

```{r, eval=F}
# ncores <- 108
# genomat_ROG <- readRDS(file=paste0(project_path, "data/genomat_restofgenome_CASS.RDS"))
# genomat_ROG$compute_group <- as.numeric(cut_number(c(1:nrow(genomat_ROG)), ncores))
# saveRDS(genomat_ROG, file=paste0(project_path, "data/genomat_restofgenome_108cores.RDS"))
...
SNP_GWAS_bivar_function <- function(section){
  ...
  #read in data
  genomat_ROG <<- readRDS(file=paste0(project_path, "data/genomat_restofgenome_CASS.RDS"))
  Amatrix_notsing <<- readRDS(file=paste0(project_path, "data/Amatrix_notsingular_CASS.RDS"))
  BLUPs_hap_data <<- readRDS(file=paste0(project_path, "data/BLUPs_and_hap_data_CASS.RDS"))
  Amat_haps_PSY2_notsing <<- readRDS(file=paste0(project_path, "data/Amat_PSY2haps_notsingular_CASS.RDS"))
  Amat_haps_IDMs_notsing <<- readRDS(file=paste0(project_path, "data/Amat_IDMhaps_notsingular_CASS.RDS"))
  ...
  #get the last SNP that was run to resume where we left off
  # previousprogress <- read.table(file = paste0(project_path, "output/progressbar/section_", section, ".txt"))
  # lastSNPrun <- tail(previousprogress, n=1)$V2
  totalSNP <- length(section_SNPs$SNP)
  lastSNPrun <- 0

  for(i in (lastSNPrun+1:totalSNP)){
    ...
    modeldata <- left_join(BLUPs_hap_data, SNPidata, by="germplasmName") %>% 
      mutate(germplasmName = as.factor(germplasmName)) %>%
      mutate(TCICHK = scale(TCICHK), DM=scale(DM))
    
    #model calculating SNP effect on bivariate response
    SNPimodel <- asreml(data = modeldata, 
        formula(paste0("cbind(TCICHK, DM) ~ trait + ", SNPi)), 
         random = ~ at(trait):vm(germplasmName, Amatrix_notsing) + 
          ...
          and(vm(PSY2_HAPB, Amat_haps_PSY2_notsing)) +
           at(trait):vm(IDMs_HAPA, Amat_haps_IDMs_notsing) + 
          and(vm(IDMs_HAPB, Amat_haps_IDMs_notsing)), maxit=20)
  ...
  saveRDS(SNPeff_storage, file=paste0(project_path, 
                  "output/Bivariate_SNPeffects_section", 
                  section, ".RDS"))
  saveRDS(WaldTest_storage, file=paste0(project_path, 
                  "output/Bivariate_SNPWaldTest_section", 
                  section, ".RDS"))
 
  cat(paste0(SNPi, ", ", i, " of ", nrow(section_SNPs), " in section ", section, "\n"),
      file = paste0(project_path, "output/progressbar/section_", section, ".txt"), append=T)
...
cl <- makeCluster(ncores)
# test each SNP in chromosome 
results <- parSapply(cl, sections, SNP_GWAS_bivar_function)
# close cluster object
stopCluster(cl)
```
...
# Combine results from the parallel computation sections:
library(tidyverse)
project_path <- "/workdir/ssv42/"


#combine all SNP effects
all_SNP_results <- list()
all_Wald_results <- list()
for (i in seq_along(sections)){ #append each section together into one list
  section <- sections[i]
  
  SNPeffs_i <- readRDS(file=paste0(project_path,
                                    "mvGWAS/output/Bivariate_SNPeffects_section", section, ".RDS"))
  Waldtests_i <- readRDS(file=paste0(project_path,
                                      "mvGWAS/output/Bivariate_SNPWaldTest_section", section, ".RDS"))
...
saveRDS(all_SNP_results, file=paste0(project_path,
                                     "mvGWAS/output/Bivariate_allSNPeffects_sections", sections[1], "-", sections[length(sections)], "_",
                                     format(Sys.time(), "%m%d"), ".RDS"))
saveRDS(all_Wald_results, file=paste0(project_path,
                                      "mvGWAS/output/Bivariate_allSNPWaldTests_sections", sections[1], "-", sections[length(sections)], "_",
                                      format(Sys.time(), "%m%d"), ".RDS"))



#Transform data so SNPs are in rows and Beta estimates and pvalues are in columns
all_SNP_results_df <- as.data.frame(matrix(nrow = length(all_SNP_results), ncol = 9))
colnames(all_SNP_results_df) <- c("Beta_SNP", "Beta_SNP_SE", "Zratio", "Df", "denDF", "F.inc", "F.con", "Margin", "Pr")
                              

for (i in 1:length(all_SNP_results)){ #this takes several minutes
  
  Betas_i <- t(all_SNP_results[[i]][1,])
  Wald_i <- all_Wald_results[[i]][2,]
  
  all_SNP_results_df[i,] <- cbind(Betas_i, Wald_i)
  
  rownames(all_SNP_results_df)[i] <- rownames(all_SNP_results[[i]])[1]
}
...
saveRDS(all_SNP_results_df, file=paste0(project_path,
             "mvGWAS/output/Bivariate_allSNPeffects_df_", format(Sys.time(), "%m%d"), ".RDS"))


```

Copy results back to cbsurobbins:

scp ./Bivariate_all* ssv42@cbsurobbins.biohpc.cornell.edu:/workdir/ssv42/mvGWAS/output/
scp *0608* ssv42@cbsurobbins.biohpc.cornell.edu:/workdir/ssv42/mvGWAS/output/mvGWAS_asreml_output/ #version with PCs





Fit trivariate TCICHK/TCHART/DM null model to compare Vg with GEMMA:
```{r}
#Prepare model dataset
i <- 1 #picking first SNP doesn't matter here for null model
SNPi <- section_SNPs$SNP[i]
SNPidata <- as.data.frame(t(section_SNPs[i, -c(1:3)]))
colnames(SNPidata) <- paste0(SNPi)
SNPidata$germplasmName <- gsub("\\.","-", rownames(SNPidata))
...
#Null trivariate model
nullTriModel <- asreml(data = modeldata,
        formula(paste0("cbind(TCICHK, TCHART, DM) ~ trait + 
                       trait:S1_24155522 + trait:S1_24155522_dom + PC1 + PC2 + PC3 + PC4 + PC5")),
        random = ~ corgh(trait):vm(germplasmName, Ginv_ROG),
        residual = ~units:corgh(trait), maxit=20)

nullTriModel <- update.asreml(nullTriModel)

summary(nullTriModel, coef=T)
...
Compare p-value calculation methods:
```{r}
# # Method 1 - Calculate p-values using the joint Z-score
# Z1Z2cor <- cor(all_SNP_results_df$Z_TC, all_SNP_results_df$Z_DM)
# B1B2cor <- cor(all_SNP_results_df$Beta_TC, all_SNP_results_df$Beta_DM)
# 
# all_SNP_results_df$jointZ <- all_SNP_results_df$Z_TC * sqrt(1 - Z1Z2cor^2) + all_SNP_results_df$Z_DM * Z1Z2cor
# all_SNP_results_df$jointP <- pnorm(abs(all_SNP_results_df$jointZ), lower.tail=FALSE)
# all_SNP_results_df$jointPx2 = all_SNP_results_df$jointP*2
# qqman::qq(all_SNP_results_df$jointPx2, main="bivariate Z-score")
# 
# # # Generate null distribution of p-values under this distribution (may be not uniform) and check qq plot:
# # using null joint Z
# cov_matrix <- matrix(c(1, B1B2cor, B1B2cor, 1), nrow=2, ncol=2)
# nullZs <- mvrnorm(n=nrow(all_SNP_results_df), mu=c(0,0), Sigma=cov_matrix)
# Z1Z2cor_null <- cor(nullZs[,1], nullZs[,2])
# jointZ_null <- nullZs[,1] * sqrt(1 - Z1Z2cor_null^2) + nullZs[,2] * Z1Z2cor_null
# jointP_null <- 2 * pnorm(abs(jointZ_null), lower.tail=FALSE)
# 
# # ## Make Q-Q plot:
# # qqplotdf <- data.frame(exp=sort(-log10(jointP_null)), obs=sort(-log10(all_SNP_results_df$jointPx2)))
# # ggplot(qqplotdf, aes(exp, obs)) +
# #   geom_point() +
# #   xlab("Expected -log10(p)") +
# #   ylab("Observed -log10(p)") +
# #   ggtitle("Q-Q plot for mvLMM") +
# #   geom_abline(intercept = 0, slope = 1, color="red")
# 
# 
# # Method 2 - Calculate p-values using the pmvnorm() function and the two Z-scores:
# all_SNP_results_df$P_Ttest <- NA
# betacor <- cor(all_SNP_results_df$Beta_TC, all_SNP_results_df$Beta_DM)
# for(i in 1:nrow(all_SNP_results_df)){
#   cov_matrix <- matrix(c(1, betacor, betacor, 1), nrow=2, ncol=2)
#   lower <- c(-Inf, -Inf)  # lower bounds
#   upper <- c(all_SNP_results_df$Z_TC[i], all_SNP_results_df$Z_DM[i])
#   p_value <-  1 - mvtnorm::pmvnorm(mean=c(0, 0), corr=cov_matrix, lower, abs(upper))[1]
#   all_SNP_results_df$P_Ttest[i] <- p_value
# }
# 
# # Generate null distribution of p-values under this distribution and check qq plot:
# nullZs <- mvrnorm(n=nrow(all_SNP_results_df), mu=c(0,0), Sigma=cov_matrix)
# nullPs <- c()
# for(i in 1:nrow(nullZs)){
#   p_val <- 1 - mvtnorm::pmvnorm(mean=c(0, 0), corr=cov_matrix, lower, abs(nullZs[i,]))[1]
#   nullPs <- c(nullPs, p_val)
# }
# 
# hist(nullPs, main="distribution of p-values for null but correlated Z-scores")

# 
# ## Make Q-Q plot:
# qqplotdf <- data.frame(exp=sort(-log10(nullPs)), obs=sort(-log10(all_SNP_results_df$P_Ttest)))
# ggplot(qqplotdf, aes(exp, obs)) +
#   geom_point() +
#   xlab("Expected -log10(p)") +
#   ylab("Observed -log10(p)") +
#   ggtitle("Q-Q plot for mvMLMM") +
#   geom_abline(intercept = 0, slope = 1, color="red")

# ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_TCDM_QQplot_", dataset, "_", format(Sys.time(), "%m%d_%H%M"), ".jpg"))

# Method 3 - Calculate p-values using the bivariate Z-score distance from origin:
all_SNP_results_df$Zdistance <- sqrt(all_SNP_results_df$Z_TC^2 + all_SNP_results_df$Z_DM^2)
all_SNP_results_df$Zdist_P <- 2*pnorm(all_SNP_results_df$Zdistance, lower.tail=F)


ggplot(all_SNP_results_df, aes(Z_TC, Z_DM, color=-log10(P_Ttest))) +
  geom_point() +
  ggtitle("p-value with mvpnorm() and separate Z-scores")

ggplot(all_SNP_results_df, aes(predicted.value_TCICHK/sqrt(std.error_TCICHK), predicted.value_DM/sqrt(std.error_DM),
       color=-log10(P))) +
  geom_point() +
  ggtitle("mvWald")

ggplot(all_SNP_results_df, aes(Z_TC, Z_DM, color=-log10(jointPx2))) +
  geom_point()+
  ggtitle("p-value for joint Z-score")

ggplot(all_SNP_results_df, aes(Z_TC, Z_DM, color=-log10(Zdist_P))) +
  geom_point() +
  ggtitle("p-value with bivariate Z-score distance from origin")

compare_pcalc_methods <- left_join(all_SNP_results_df, all_SNP_results_Bivar)

ggplot(compare_pcalc_methods, aes(Pr, P_Ttest)) +
  geom_point(size=0.1) +
  xlab("p-value from single Wald test") +
  ylab("p-value from mvpnorm() and separate Z-scores")

ggplot(compare_pcalc_methods, aes(Pr, jointPx2)) +
  geom_point(size=0.1) +
  xlab("p-value from single Wald test") +
  ylab("p-value for joint Z-score")

ggplot(compare_pcalc_methods, aes(Z_TC, Z_DM, color=-log10(Pr))) +
  geom_point() +
  ggtitle("p-value from single Wald test")

ggplot(compare_pcalc_methods, aes(Z_TC, Z_DM, color=abs(jointZ))) +
  geom_point() +
  ggtitle("Single versus joint Z-scores")

ggplot(compare_pcalc_methods, aes(Z_TC, Z_DM, color=abs(jointZ))) +
  geom_point() +
  ggtitle("Single versus joint Z-scores")

ggplot(compare_pcalc_methods, aes(Beta_TC, Beta_DM, color=Beta_SNP)) +
  geom_point()

ggplot(compare_pcalc_methods, aes(Zratio, jointZ)) +
  geom_point()

ggplot(compare_pcalc_methods, aes(-log10(Pr_TC), -log10(Pr_DM), color=-log10(P_Ttest))) +
  geom_point()
```


Alt F test construction:
$$F_0 = \frac{(SSR_r - SSR_{ur})/q}{SSR_{ur}/(n-(k+1))}$$
Note: I don't think this F-test is proper for a mixed linear model.
```{r, eval=F}
URmodel <- asreml(data = modeldata, 
        formula(paste0("cbind(TCICHK, DM) ~ trait + at(trait):", SNPi)), 
         random = ~ at(trait):vm(germplasmName, Amatrix_notsing) + 
           at(trait):vm(PSY2_HAPA, Amat_haps_PSY2_notsing) + 
          and(vm(PSY2_HAPB, Amat_haps_PSY2_notsing)) +
           at(trait):vm(IDMs_HAPA, Amat_haps_IDMs_notsing) + 
...
SSRr <- sum(Rmodel$residuals^2, na.rm=T) #sum of the squared residuals of the restricted model
SSRur <- sum(URmodel$residuals^2, na.rm=T) #sum of the squared residuals of the unrestricted model
q <- 2 #number of restrictions (the number of coefficients being jointly tested)
n <- 378 #number of observations
k <- 8 # number of independent variables in the unrestricted model 
  
F0 <- ( (SSRr-SSRur)/q ) / (SSRur/ (n-(k+1)))
pf(F0, df1= q, df2= n - (k+1) )
```




Plot p-values for individual traits:
```{r}
MVresults_allPs <- MVresults_sort %>% pivot_longer(cols=c(Pr_TC, Pr_DM, jointP)) %>% as.data.frame()

## Plot Manhattan with joint p-values
ggplot(MVresults_allPs, aes(x=POS, y=-log10(value))) +
  geom_point(aes(color=name), size=0.5) +
  labs(y="-log10(p)", x="Chromosome physical position") +
  ggtitle(paste0("MLMM GWAS for TC and DM")) +
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4) +
  facet_grid(. ~ CHROM, scales = "free_x", switch="x") +
    theme(strip.background = element_rect(fill = "white"),
          axis.text.x = element_blank(),  # Remove x-axis text
...
#set positions for axis ticks centered by chromosome, when plotting SNPs as factors
xaxisticks <- MVresults_sort %>% group_by(CHROM) %>% summarise(midpt = nth(SNP_ID, round(n()/2))) %>% as.data.frame() 


## Plot Manhattan with separate p-wald values for TC and DM
ggplot(MVresults_sort, aes(x=SNP_ID, y=-log10(Pr_TC))) +
  geom_point(color=colPalette[2], size=0.5, alpha=0.5) +
  geom_point(aes(x=SNP_ID, y=-log10(Pr_DM)), color=colPalette[1], size=0.5, alpha=0.5) +
  scale_color_manual(values=colPalette) +
  labs(y="-log10(p)") +
  scale_x_discrete(name="chromosome", breaks=xaxisticks$midpt, labels=c(1:18)) +
  ggtitle(paste0("Asreml mvGWAS for TC and DM; ", dataset, " dataset")) +
  geom_vline(xintercept="1_24155522", color="orange", lty="dashed") + #causal SNP
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4)

ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_TCDM_", dataset, "_", format(Sys.time(), "%m%d_%H%M"), ".jpg"))



## Plot Manhattan with raw p-wald values for TC only
ggplot(MVresults_sort, aes(x=SNP_ID, y=-log10(Pr_TC))) +
  geom_point(aes(color=CHROM), size=0.5) +
  scale_color_manual(values=colPalette) +
  labs(y="-log10(p)") +
  scale_x_discrete(name="chromosome", breaks=xaxisticks$midpt, labels=c(1:18)) +
  ggtitle(paste0("Asreml mvGWAS for TC and DM; TC p-values ", dataset, " dataset")) +
  guides(color="none")+ 
  geom_vline(xintercept="1_24155522", color="orange", lty="dashed") + #causal SNP PSY2
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4)

ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_TConly_", dataset, "_", format(Sys.time(), "%m%d_%H%M"), ".jpg"))


## Plot Manhattan with raw p-wald values for DM only
ggplot(MVresults_sort, aes(x=SNP_ID, y=-log10(Pr_DM))) +
  geom_point(aes(color=CHROM), size=0.5) +
  scale_color_manual(values=colPalette) +
  labs(y="-log10(p)") +
  scale_x_discrete(name="chromosome", breaks=xaxisticks$midpt, labels=c(1:18)) +
  ggtitle(paste0("Asreml mvGWAS for TC and DM; DM p-values ", dataset, " dataset")) +
  guides(color="none")+ 
  geom_vline(xintercept="1_24155522", color="orange", lty="dashed") +  #PSY2 causal SNP
  geom_hline(yintercept = -log10(Gao.cutoff), linetype="dashed", lwd=0.4)

ggsave(filename=paste0(project_path, "figures/asreml_mvGWAS_DMonly_", dataset, "_", format(Sys.time(), "%m%d_%H%M"), ".jpg"))
...
#OLD
```{r,eval=F}
#Chromosome 1 peak model comparison results
#S1_23493338
summary(model_comparison_allSNPs[[1]][[1]], coef=T)$coef.fixed #bivariate TCICHK & DM
summary(model_comparison_allSNPs[[1]][[2]], coef=T)$coef.fixed #TCICHK with DM covariate
summary(model_comparison_allSNPs[[1]][[3]], coef=T)$coef.fixed #DM with TCICHK covariate
summary(model_comparison_allSNPs[[1]][[4]], coef=T)$coef.fixed #TCICHK 
summary(model_comparison_allSNPs[[1]][[5]], coef=T)$coef.fixed #DM
summary(model_comparison_allSNPs[[1]][[1]], coef=T)$aic #bivariate TCICHK & DM
summary(model_comparison_allSNPs[[1]][[2]], coef=T)$aic #TCICHK with DM covariate
summary(model_comparison_allSNPs[[1]][[3]], coef=T)$aic #DM with TCICHK covariate
summary(model_comparison_allSNPs[[1]][[4]], coef=T)$aic #TCICHK 
summary(model_comparison_allSNPs[[1]][[5]], coef=T)$aic #DM

#S1_26700297
summary(model_comparison_allSNPs[[2]][[1]], coef=T)$coef.fixed #bivariate TCICHK & DM
summary(model_comparison_allSNPs[[2]][[2]], coef=T)$coef.fixed #TCICHK with DM covariate
summary(model_comparison_allSNPs[[2]][[3]], coef=T)$coef.fixed #DM with TCICHK covariate
summary(model_comparison_allSNPs[[2]][[4]], coef=T)$coef.fixed #TCICHK 
summary(model_comparison_allSNPs[[2]][[5]], coef=T)$coef.fixed #DM

#both SNPs together:
summary(model_comparison_allSNPs[[4]], coef=T)$coef.fixed
summary(model_comparison_allSNPs[[4]], coef=T)$aic
print(WaldTest_1a)
print(WaldTest_1b)


#Chromosome 10 peak model comparison results
summary(model_comparison_allSNPs[[3]][[1]], coef=T)$coef.fixed #bivariate TCICHK & DM
summary(model_comparison_allSNPs[[3]][[2]], coef=T)$coef.fixed #TCICHK with DM covariate
summary(model_comparison_allSNPs[[3]][[3]], coef=T)$coef.fixed #DM with TCICHK covariate
summary(model_comparison_allSNPs[[3]][[4]], coef=T)$coef.fixed #TCICHK 
summary(model_comparison_allSNPs[[3]][[5]], coef=T)$coef.fixed #DM

summary(model_comparison_allSNPs[[3]][[1]], coef=T)$aic #bivariate TCICHK & DM
summary(model_comparison_allSNPs[[3]][[2]], coef=T)$aic #TCICHK with DM covariate
summary(model_comparison_allSNPs[[3]][[3]], coef=T)$aic #DM with TCICHK covariate
summary(model_comparison_allSNPs[[3]][[4]], coef=T)$aic #TCICHK 
summary(model_comparison_allSNPs[[3]][[5]], coef=T)$aic #DM
```
