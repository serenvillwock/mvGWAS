...
trialdatall <- renameAndSelectCols(traitabbrevs, indata=rawtrialdata_staged, 
  ...
  mutate(CMD1S=ifelse(CMD1S<1 | CMD1S>5,NA,CMD1S), #remove any nonsensical values for ordinal traits
         ...
         DM=ifelse(is.na(DM) & !is.na(DMSPGRAV), DMSPGRAV, DM), #use DMSPGRAV as DM if DM is missing but DMSPGRAV is not
...
#print selected trial info for table S1
...
write.csv(trialinfo, file=paste0(project_path, "data/selected_trial_samplesize_info.csv"))
...
Sample sizes and trials observed per accession:
```{r}
trialdat <- readRDS(file=paste0(project_path, "data/trialdata_subset_", dataset, "_TCICHK_", "H2filterFALSE.RDS"))

accession_summary <- trialdat %>% mutate(stageorder = case_when(selStage == "GG" ~ 1, 
                                                                selStage == "CET" ~ 1,
                                                                selStage == "PYT" ~ 2,
                                                                selStage == "AYT" ~ 3,
                                                                selStage == "UYT" ~ 4, T ~ NA)) %>%
  group_by(germplasmName) %>%
  summarise(nobs = n(), maxselstage = max(stageorder), trials = paste0(studyName, collapse = ","))


                
saveRDS(accession_summary, file=paste0(project_path, "data/trialdata_samplesize_peraccession_wselstage.RDS"))
```
...
for(i in seq_along(traitnames)){
  ...
  nobs_traiti <- trialdat %>% filter( !is.na(get(traiti)) ) %>% 
    filter(germplasmName %in% genotyped_accessions) %>% nrow()
  
  traitcounts[i] <- ngenos_trait
  traitobscounts[i] <- nobs_traiti

  #cat(traiti, ngenos_trait, "\n")
}

TraitCounts <- data.frame(TraitAbbrev = traitnames, NGenotypes = as.numeric(traitcounts), Nobservations = as.numeric(traitobscounts)) %>%
...
for(i in seq_along(selectedtraits)){ 
  ...
  hist(trialdat[,traiti], main= paste0(traiti, "; n=", 
                                       sum(!is.na(trialdat[,traiti]))), xlab=paste0(traiti))  
}


hist(trialdat_genod$TCICHK)
```
...
# Create breaks
xbreaks <- seq(min(df$Y1, na.rm=T), max(df$Y1, na.rm=T), length.out = xbins + 1)
ybreaks <- seq(min(df$Y2, na.rm=T), max(df$Y2, na.rm=T), length.out = ybins + 1)

# Cut the values into bins
df$x_bin <- cut(df$Y1, breaks = xbreaks)
df$y_bin <- cut(df$Y2, breaks = ybreaks)

# Count occurrences per bin
z_table <- df %>%
  count(x_bin, y_bin)

# Create a full grid to fill in missing combinations with 0
z_table_full <- expand.grid(
  x_bin = levels(df$x_bin),
  y_bin = levels(df$y_bin)
) %>%
  left_join(z_table, by = c("x_bin", "y_bin")) %>%
  mutate(n = ifelse(is.na(n), 0, n))

# Get bin centers for plotting
x_centers <- sapply(strsplit(as.character(z_table_full$x_bin), ","), function(x) {
  mean(as.numeric(gsub("[^0-9.-]", "", x)))
})
y_centers <- sapply(strsplit(as.character(z_table_full$y_bin), ","), function(x) {
  mean(as.numeric(gsub("[^0-9.-]", "", x)))
})

# Convert to matrix for z values
z_matrix <- matrix(z_table_full$n, 
                   nrow = length(unique(x_centers)), 
                   ncol = length(unique(y_centers)))

# Plot 3D surface
plot_ly(x = sort(unique(x_centers)), 
        y = sort(unique(y_centers)), 
        z = z_matrix) %>%
  add_surface() %>%
  layout(scene = list(
    xaxis = list(title = "Y1"),
...
for(i in seq_along(selectedtraits)){ #go through each trait
  ...
  if(!inherits(trymodelfit, "error") && trymodelfit$converge == TRUE){ #if the model successfully fit,
    ...
    N <- nrow(modi$mf)
    threshold <- qt(p =  1 - 0.001 / (2 * N), df = (N - 2))
...
#consider GG trials as CET trials for purpose of het. error variance calculations
trialdat_cleaned$selStage[trialdat_cleaned$selStage == "GG"] <- "CET"
trialdat_cleaned$selStage <- droplevels(trialdat_cleaned$selStage)


#initiate storage 
model_storage <- list() 
BLUP_storage <- data.frame(germplasmName = sort(unique(trialdat_cleaned$germplasmName))) 
weights_storage <- data.frame(germplasmName = sort(unique(trialdat_cleaned$germplasmName))) 
...
for(i in seq_along(selectedtraits)){ #go through each trait
  ...
  trymodelfit <- tryCatch(
    ...
    # homogeneous error variance version: 
    # OGmodi <- asreml(data=trialdat_cleaned, trace=F,
    #                fixedformula,
    #                random = ~ germplasmName +
    #                  germplasmName:locationName:studyYear + studyName:blockNumber,
    #                 residual = ~ units, #homogeneous error variance
    #                na.action = na.method(y="omit")) ,
    
    # heterogeneous error variance version:
    modi <- asreml(data=trialdat_cleaned[order(trialdat_cleaned$selStage), ], trace=F,
                   fixedformula,
                   random = ~ germplasmName + 
                     germplasmName:locationName:studyYear + studyName:blockNumber,
                    residual = ~ dsum(~units | selStage), #heterogeneous error variances by selection stage
                   na.action = na.method(y="omit")) ,
    
    #compare hom. and het. error variance models:
    #lrt.asreml(modi, OGmodi)
    #for TCICHK: LR-statistic= 71.934, df= 4, p < 9.9e-16
    #for DM: LR-statistic= 85.392, df= 4, p < 2.2e-16
    #in both cases the het. err. var model had lower AIC/BIC
  ...
  if(!inherits(trymodelfit, "error") && trymodelfit$converge == TRUE){ #if the model converged:
    ...
    h2 <- Vg / sum(summary(modi)$varcomp[,1])
    ...
    BLUPs_i <- randomeffs[grepl('germplasmName', rownames(randomeffs)) & !grepl('locationName', rownames(randomeffs)),] %>%
      ...
      mutate(PEV=std.error^2, 
             ...
             weight = (1-h2) / (0.1 + ((1-REL)/REL)*h2) ) %>% #calculate weight as inverse of BLUP reliability for GWAS model error variance
      mutate(germplasmName = gsub("germplasmName_","",rownames(.)))
    
      #store de-regressed BLUPs
    BLUPs_tostore <- BLUPs_i %>% dplyr::select(germplasmName, drgBLUP, weight) %>%
      rename_with(~ paste0(traiti), drgBLUP) %>%
      rename_with(~ paste0(traiti, "_weight"), weight)
    BLUP_storage <- left_join(BLUP_storage, BLUPs_tostore, by="germplasmName") 
    
    #calculate BLUP weights for second-stage model
    # weights_tostore <- BLUPs_i %>% dplyr::select(germplasmName, weight) %>%
    #   rename_with(~ paste0(traiti, "_weight"), weight)
    # weights_storage <- left_join(weights_storage, weights_tostore, by="germplasmName")
    ...
    #Ve <- summary(modi)$varcomp["units!R", "component"] #homogeneous error variance
    
    # heterogeneous error variance version:
    #inverse-variance weighting of residual errors
      VCET <- (summary(modi)$varcomp["selStage_CET!R", 1]) / (1*1) #1 plot, 1 location reps
      VCETv <- (summary(modi)$varcomp["selStage_CET!R", 2])^2
      VPYT <- (summary(modi)$varcomp["selStage_PYT!R", 1]) / (2*1) #2 plot reps, 1 location reps
      VPYTv <- (summary(modi)$varcomp["selStage_PYT!R", 2])^2
      VAYT <- (summary(modi)$varcomp["selStage_AYT!R", 1]) / (3*1) #3 plot reps, 1 location reps
      VAYTv <- (summary(modi)$varcomp["selStage_AYT!R", 2])^2
      VUYT <- (summary(modi)$varcomp["selStage_UYT!R", 1]) / (3.3*2.5) #3-4 plot reps, 2-3 location reps
      VUYTv <- (summary(modi)$varcomp["selStage_UYT!R", 2])^2
      VRweightedavg <- sum( VCET/VCETv, VPYT/VPYTv, VAYT/VAYTv, VUYT/VUYTv ) / 
        sum( 1/VCETv, 1/VPYTv, 1/VAYTv, 1/VUYTv )
 
    H2 <- Vg / sum(Vg + (Vgly/harmonicMeanEnv) + VRweightedavg)
...
### Save BLUPs
saveRDS(BLUP_storage, file=paste0(project_path, "data/alltrait_scaled_drgBLUPs_rev_", dataset, "_", TCtrait, "_", "H2filter", filter_trial_byH2, ".RDS"))
saveRDS(H2_storage, file=paste0(project_path, "data/alltrait_scaled_H2_rev_", dataset, "_", TCtrait, "_", "H2filter", filter_trial_byH2, ".RDS"))
saveRDS(H2_cullis_storage, file=paste0(project_path, "data/alltrait_scaled_H2_cullis_rev_", dataset, "_", TCtrait, "_", "H2filter", filter_trial_byH2,".RDS"))
saveRDS(model_storage, file=paste0(project_path, "data/BLUPmodel_scaled_storage_rev_", dataset, "_", TCtrait, "_", "H2filter", filter_trial_byH2, ".RDS"))
#saveRDS(weights_storage, file=paste0(project_path, "data/alltrait_scaled_accession_errorvar_weights_v1.RDS"))
...
```{r, warning=FALSE, message=FALSE}
#note: "_rev" tag refers to revision version (reviewer #2 response to het. error variance issue)


H2_storage<- readRDS(file=paste0(project_path, "data/alltrait_scaled_H2_rev_CASS_TCICHK_H2filter", filter_trial_byH2, ".RDS"))
BLUP_storage <- readRDS(file=paste0(project_path, "data/alltrait_scaled_drgBLUPs_rev_CASS_TCICHK_H2filter", filter_trial_byH2, ".RDS"))
model_storage <- readRDS(file=paste0(project_path, "data/BLUPmodel_scaled_storage_rev_CASS_TCICHK_H2filter", filter_trial_byH2, ".RDS"))
...
genotyped_accessions <- readRDS(file="./data/genomat_rev_plusPSY2KASP_germplasmNames.RDS")
...
saveRDS(BLUP_storage_genod, file=paste0(project_path, "data/BLUPmatrix_rev_genodsubset.RDS"))
...
ggsave(file=paste0(project_path, "figures/TraitScaledCorrMatrix_rev.jpg"), bg="white")
...
ggsave(file=paste0(project_path, "figures/TCDM_scaled_cor_plot_all_rev.jpg"))



#Genetic correlation between TC and DM in yellow-only dataset
ggplot(data=BLUP_storage %>% filter(TCICHK>=-1), aes(TCICHK, DM)) +
  geom_point() +
...
trialdat_subset_colorsum <- trialdat_subset %>% filter(germplasmName %in% BLUPs$sample_names) %>%
  group_by(germplasmName) %>%
  summarise(meanTCHART = mean(TCHART, na.rm=T), meanTCICHK = mean(TCICHK, na.rm=T), meanDM = mean(DM, na.rm=T)) %>%
...
ggplot(trialdat_genod, aes(TCICHK, DM)) +
  geom_point(aes(fill=TCHART), pch=21, color="dark gray", size=2, aes=0.5) +
  scale_fill_gradient2(low="white", mid="#ffec5c", high="dark orange", midpoint=4)

# Fit phenotypic correlation 
lm_fit <- lm(DM ~ TCICHK, data = trialdat_genod)
slope <- coef(lm_fit)[["TCICHK"]]
intercept <- coef(lm_fit)[["(Intercept)"]]
r.squared <- summary(lm_fit)$r.squared


ggplot(trialdat_genod, aes(TCICHK, DM)) +
  geom_point(aes(fill=TCICHK), pch=21, color="dark gray", size=2.5, alpha=1) +
  scale_fill_gradient2(low="white", mid="#fff5a8", high="#fac746", midpoint=15) +
   labs(x = expression("TCICHK ("*mu*"g/g FW)"), y = "DM (%)",
    title = "Raw phenotypic distributions") +
  ylim(c(0,50)) +
  geom_smooth(method = "lm", se = FALSE, linewidth=0.3, lty="dashed", color="black") +
  annotate("text", x = 23.5, y = 48, hjust = 0, vjust = 1.5, size = 4,
           label = paste0("y = ", round(slope, 3), "x + ", round(intercept, 1))) +
    annotate("text", x = 23.5, y = 44, hjust = 0, vjust = 1.5, size = 4,
            label = paste0("r² = ", round(r.squared, 3))) 


ggsave("./figures/raw_phenotypic_distributions_TCICHK_DM.png", height=5, width=7, units="in")

range(trialdat_genod$TCICHK, na.rm=T)
range(trialdat_genod$DM, na.rm=T)
